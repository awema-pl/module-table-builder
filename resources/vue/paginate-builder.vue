<template>
    <div class="pager" v-if="meta">
        <span class="pager__caption">{{$lang.PAGINATE_RECORD}} {{ meta.from }}-{{ meta.to }} {{$lang.PAGINATE_OF}} {{ meta.total }}</span>

        <div class="pager__middle" v-if="paginate">
            <div class="pager__links">
                <a
                    :href="getStringified(currentPage - 1)"
                    class="pager__arr-left awema-spa-ignore"
                ...
                <div class="btn-group">
                    <template v-for="(p, i) in paginate">
                        <a  v-if="p"
                            :key="i"
                            class="btn has-wave awema-spa-ignore"
                ...
                <a
                    :href="getStringified(currentPage + 1)"
                    class="pager__arr-right awema-spa-ignore"
                    @click.stop.prevent="setPage(currentPage + 1)"
                >
...
export default {
    ...
    methods: {
        ...
        getStringified(page) {
            return this.$route.path + '?' + AWEMA.utils.stringifyQuery(Object.assign({}, this.$route.query, {page})) + this.$route.hash
        }
    },
